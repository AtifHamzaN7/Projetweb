...
public class AiPocControllerWebMvcTest {
    ...
    @Test
    public void testGetSecondEndpoint_Returns200() throws Exception {
        mockMvc.perform(get("/ai/thesecond")
                .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("ok"));
    }
    ...
    @Test
    public void testGetSecondEndpoint_BadRequest_Returns400() throws Exception {
        mockMvc.perform(get("/ai/thesecond?bad=true")
                .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error").value("bad request"));
    }
